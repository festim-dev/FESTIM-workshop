...
Objectives:
* Impose a simple surface reaction boundary condition
* recombination and dissociation
* Multi-isotope

+++

## Simple surface reaction BC ##

A reaction is defined by specifying the reactants (`reactant`), gas pressure (`gas_pressure`), forward/backward rate constants (`k_r0` and `k_d0`), and rate activation energies (`E_kr` and `E_kd`).

```{note}
Reaction rates are given as Arhennius laws.
```

To impose the folllowing surface reaction:
...
$$ H + H \overset{K_r}{\underset{K_d}{\rightleftharpoons}} H_2 $$
...
Surface reactions can involve multiple hydrogen isotopes, enabling the modelling of more complex interactions between species. For example, in a system with both mobile hydrogen and tritium, various molecular recombination pathways may occur at the surface, resulting in the formation of $H_2$, $T_2$, and $HT$:
